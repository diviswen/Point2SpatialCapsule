...
def netVLAD(inputs, num_clusters, is_training, bn=True, bn_decay=0.999):
    ...
    # soft-assignment.
    s = tf.contrib.layers.fully_connected(inputs=inputs, num_outputs=num_clusters, scope='assignment',biases_initializer=None)
    if bn:
        s = slim.batch_norm(s,
                  center=True,
                  scale=True,
    ...
    # VLAD core.
    C = tf.get_variable('cluster_centers', [1, 1, K, D],
                        dtype=inputs.dtype)

    v = tf.expand_dims(inputs, -2) + C
    v = a * v
